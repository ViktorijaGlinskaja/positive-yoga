...
const PricingPlanBox = ({ pricingPlan, setPricingPlan }) => {
    ...
    return (
        <Box sx={{ margin: '16px 0px', display: 'grid', gap: '8px' }} >
            {pricingInfo.map((plan, index) => <PricingPlan selected={pricingPlan === index} onClick={() => setPricingPlan(index)} key={plan.id} title={plan.title} price={plan.price} discount={plan.discount} oldPrice={plan.oldPrice} newPrice={plan.newPrice} frequency={plan.frequency} />)}
        </Box>
    )
};
